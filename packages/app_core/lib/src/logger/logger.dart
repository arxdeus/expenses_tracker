...
import 'package:app_core/src/logger/ansi_color.dart';
...
base class LogOptions {
  /// {@macro log_options}
  const LogOptions({this.showTime = true, this.logInRelease = false, this.level = LoggerLevel.verbose, this.formatter});
...
base class LogMessage {
  /// {@macro log_message}
  const LogMessage({required this.message, required this.logLevel, this.error, this.stackTrace, this.time});
...
final class LoggerLogging extends Logger {
  ...
  L runLogging<L>(L Function() fn, [LogOptions options = const LogOptions()]) {
    ...
    _logger.onRecord.where((event) => event.loggerName == 'Logger').listen((event) {
...
String _formatLoggerMessage({required LogMessage log, required LogOptions options}) {
  ...
  return log.logLevel.color(buffer.toString());
}
...
extension on logging.LogRecord {
  ...
  LogMessage toLogMessage() =>
      LogMessage(message: message, error: error, stackTrace: stackTrace, time: time, logLevel: level.toLoggerLevel());
}
...
extension on LoggerLevel {
  ...
  String Function(String) get color => switch (this) {
    LoggerLevel.error => red,
    LoggerLevel.warning => orange,
    LoggerLevel.info => green,
    LoggerLevel.debug => yellow,
    LoggerLevel.verbose => cyan,
  };
}
