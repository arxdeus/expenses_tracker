...
class _PieChartState extends State<PieChart> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.diameter case final double value when value > .0) chart = _fit(child: chart, diameter: value);
    return chart;
  }
}
...
class _PieChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gap = (widget.gap?.clamp(0, 64) ?? 4) / 2 / radius; // Gap between sections
